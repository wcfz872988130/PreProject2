...
public class PlayerOutLine : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		_depthShader = Shader.Find("Unlit/Depth");
		...
		_camera.cullingMask = 1 << (int)LayerMask.NameToLayer ("Player");
		_camera.enabled = false;
		_materialOcclusion = new Material(Shader.Find("Unlit/occlusion"));
		_materialStretch = new Material (Shader.Find ("Unlit/stretch"));
		_materialMix = new Material (Shader.Find("Unlit/mix"));
	...
	void OnRenderImage(RenderTexture source,RenderTexture destination)
	{
		...
		//_materialOcclusion.SetTexture ("_CameraDepthTexture",_renderTextureDepth);
